...
class PlanOptimizer:
    ...
    def optimize(
        ...
        """
        if (
            not context.execution_memory
            or not context.execution_memory.previous_executions
        ):
            return tasks
        ...
        for prev_exec in context.execution_memory.previous_executions:
            if prev_exec.get(
                "status"
            ) == "SUCCESS" and "search_issues" in prev_exec.get("tools", []):
        ...
        for task in tasks:
            ...
            if (
                tool_name in ["assign_label", "update_issue", "create_comment"]
                and cached_issue_id
            ):
