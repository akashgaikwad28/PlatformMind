---
Last Updated: 2026-07-26
Related Source Files: `src/platformmind/application`, `src/platformmind/domain`, `src/platformmind/infrastructure`
Applies To Version: 1.0.0
Maintainer: PlatformMind Team
Generated From: Repository Audit
---

# Architecture Index

## Purpose
This document serves as the directory for the PlatformMind architectural documentation. PlatformMind follows a strictly modular architecture designed around Domain-Driven Design (DDD) and Clean Architecture principles to ensure decoupling of business logic from external frameworks.

## Core Architecture
- [System Architecture](SYSTEM_ARCHITECTURE.md)
- [Clean Architecture](CLEAN_ARCHITECTURE.md)
- [Domain Model](DOMAIN_MODEL.md)
- [Application Layer](APPLICATION_LAYER.md)
- [Infrastructure Layer](INFRASTRUCTURE_LAYER.md)
- [API Layer](API_LAYER.md)

## Subsystems & Pipelines
- [Planner Architecture](PLANNER_ARCHITECTURE.md)
- [Execution Engine](EXECUTION_ENGINE.md)
- [Memory Architecture](MEMORY_ARCHITECTURE.md)
- [Learning Engine](LEARNING_ENGINE.md)
- [Capability Synthesis](CAPABILITY_SYNTHESIS.md)

## Data & Lifecycles
- [Request Lifecycle](REQUEST_LIFECYCLE.md)
- [Data Flow](DATA_FLOW.md)
- [Domain Events](DOMAIN_EVENTS.md)
- [Persistence](PERSISTENCE.md)

## Cross-Cutting Concerns
- [Dependency Graph](DEPENDENCY_GRAPH.md)
- [Security](SECURITY.md)
- [Error Handling](ERROR_HANDLING.md)
- [Design Decisions](DESIGN_DECISIONS.md)

## Related Modules
- [Architecture Decision Records (ADRs)](../adr/)
- [System Diagrams](../diagrams/)
